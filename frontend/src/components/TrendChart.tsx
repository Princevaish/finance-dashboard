...
import {
  ...
  PointElement,
  BarController, LineController, // ✅ ADD THESE
...
export interface TrendPoint { month: string; income: string; expense: string; net: string }

interface TrendChartProps { trends: TrendPoint[] }
...
export default function TrendChart({ trends }: TrendChartProps) {
  ...
  useEffect(() => {
    ...
    chartRef.current = new Chart(canvasRef.current, { type: "bar", data, options } as Parameters<typeof Chart>[1]);
  ...
  if (trends.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-48 text-gray-400">
        ...
        <p className="text-sm">No trend data yet — add records to see your chart</p>
      </div>
    );
  }
